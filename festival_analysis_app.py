...
def festival_analysis_app():
    ...
    if selected_analysis == "1. 축제 방문객 현황(총괄)":
        from press_release_app.festival.analyze_summary import analyze_summary
        analyze_summary()
    elif selected_analysis == "2. 축제 일자별 방문객 수":
        from press_release_app.festival.analyze_daily_visitor import analyze_daily_visitor
        analyze_daily_visitor()
    elif selected_analysis == "3. 시간대별 관광객 존재 현황":
        from press_release_app.festival.analyze_time_distribution import analyze_time_distribution
        analyze_time_distribution()
    elif selected_analysis == "4. 축제 전·중·후 방문객 분석":
        from press_release_app.festival.analyze_before_after import analyze_before_after
        analyze_before_after()
    elif selected_analysis == "5. 연령별 방문객 분석":
        from press_release_app.festival.analyze_age_group import analyze_age_group
        analyze_age_group()
    elif selected_analysis == "6. 외지인 이동지역 분석":
        from press_release_app.festival.analyze_mobility import analyze_mobility
        analyze_mobility()
