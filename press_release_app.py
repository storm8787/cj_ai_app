...
def generate_press_release(user_request, similar_examples):
    ...
    추가지시 = (
        ...
        f"전체 보도자료는 반드시 {길이지시}자 보다는 길게(+300가능) 작성해주세요."
    )
...
def press_release_app():
    ...
    문단수 = st.selectbox("📑 문단 수를 선택하세요", ["4개이상", "3개", "2개", "1개"])
    길이 = st.selectbox("📏 보도자료 길이", ["길게", "중간", "짧게"])
